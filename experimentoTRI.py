...
caminho = 'C:/Users/Servidor-LSI/Documents/Bruno Vicente/GitHub/basedados/'
...
for r, p in enumerate(porcentagem):
    ...
    tri = TriTraining(modelo, classificador)
    ...
    tri.fit(X_treino, Y_treino)
    
    #print('Guardando os dados')
    """ FASE TRANDUTIVA """
    acuraciat.append(accuracy_score(yu, tri.predict(U).astype('int64')))
    kappat.append(cohen_kappa_score(yu, tri.predict(U).astype('int64')))
    
    """ FASE INDUTIVA """
    acuraciai.append(accuracy_score(y_test, tri.predict(X_test).astype('int64')))
    kappai.append(cohen_kappa_score(y_test, tri.predict(X_test).astype('int64')))
...
resultado.to_csv('resultadosTRI/'+modelo+'/'+base+'_'+str(rotulados)+'.csv')